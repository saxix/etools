...
class TestStrUnicode(TestCase):
    ...
    def test_gateway_type(self):
        ...
        self.assertEqual(binary_type(gateway_type), b'xyz')
        self.assertEqual(text_type(gateway_type), u'xyz')

        gateway_type = GatewayTypeFactory.build(name=u'R\xe4dda Barnen')
        self.assertEqual(binary_type(gateway_type), b'R\xc3\xa4dda Barnen')
        self.assertEqual(text_type(gateway_type), u'R\xe4dda Barnen')
    ...
    def test_location(self):
        ...
        self.assertEqual(binary_type(location), b'R\xc3\xa4dda Barnen (xyz PCode: abc)')
        self.assertEqual(text_type(location), u'R\xe4dda Barnen (xyz PCode: abc)')
    ...
    def test_carto_db_table(self):
        ...
        self.assertEqual(binary_type(carto_db_table), b'R\xc3\xa4dda Barnen')
        self.assertEqual(text_type(carto_db_table), u'R\xe4dda Barnen')

        carto_db_table = CartoDBTableFactory.build(table_name=b'xyz')
        self.assertEqual(binary_type(carto_db_table), b'xyz')
        self.assertEqual(text_type(carto_db_table), u'xyz')
