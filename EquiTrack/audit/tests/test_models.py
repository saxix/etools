...
class TestStrUnicode(TestCase):
    ...
    def test_auditor_staff_member(self):
        ...
        self.assertEqual(binary_type(instance), b'Bugs Bunny')
        self.assertEqual(text_type(instance), 'Bugs Bunny')

        user = UserFactory.build(first_name='Harald', last_name='H\xe5rdr\xe5da')
        instance = AuditorStaffMemberFactory.build(user=user)
        self.assertEqual(binary_type(instance), b'Harald H\xc3\xa5rdr\xc3\xa5da')
        self.assertEqual(text_type(instance), 'Harald H\xe5rdr\xe5da')

    def test_purchase_order(self):
        instance = PurchaseOrderFactory.build(order_number=b'two')
        self.assertEqual(binary_type(instance), b'two')
        self.assertEqual(text_type(instance), 'two')

        instance = PurchaseOrderFactory.build(order_number='tv\xe5')
        self.assertEqual(binary_type(instance), b'tv\xc3\xa5')
        self.assertEqual(text_type(instance), 'tv\xe5')
    ...
    def test_engagement(self):
        ...
        self.assertIn(' two,', text_type(instance))

        purchase_order = PurchaseOrderFactory.build(order_number='tv\xe5')
        instance = EngagementFactory.build(agreement=purchase_order)
        self.assertIn(b' tv\xc3\xa5,', binary_type(instance))
        self.assertIn(' tv\xe5,', text_type(instance))

    def test_rick_category(self):
        instance = RiskCategoryFactory.build(header='two')
        self.assertEqual(binary_type(instance), b'RiskCategory two')
        self.assertEqual(text_type(instance), 'RiskCategory two')

        instance = RiskCategoryFactory.build(header='tv\xe5')
        self.assertEqual(binary_type(instance), b'RiskCategory tv\xc3\xa5')
        self.assertEqual(text_type(instance), 'RiskCategory tv\xe5')
    ...
    def test_risk_blueprint(self):
        ...
        self.assertEqual(binary_type(instance), b'RiskBluePrint at two')
        self.assertEqual(text_type(instance), 'RiskBluePrint at two')

        risk_category = RiskCategoryFactory.build(header='tv\xe5')
        instance = RiskBluePrintFactory.build(category=risk_category)
        self.assertEqual(binary_type(instance), b'RiskBluePrint at tv\xc3\xa5')
        self.assertEqual(text_type(instance), 'RiskBluePrint at tv\xe5')
